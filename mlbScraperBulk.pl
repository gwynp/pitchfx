...
$outputdir = "/opt/data/pitchfx";
 
use Time::Local;

#############################################################
#############################################################
# edit these variables to set the start and end dates
#############################################################
#############################################################
$startDay=1;
$startMonth=4;
...
sub createTimeStamp {
	# create a timestamp from the start and end variables
	($mon) = @_[0];
	($mday) = @_[1];
	($year) = @_[2];
	#$mon  += 1;
print "start of createTimeStamp sub  = 0,0,0,$mday,$mon,$year\n";
    $year -= 1900;
    print "mon is $mon\n";
    --$mon;
	 print "after mon is $mon\n";
    $mon = (length($mon) == 1) ? "0$mon" : $mon;
    $mday = (length($mday) == 1) ? "0$mday" : $mday;
	print "in creatTimeStamp sub = 0,0,0,$mday,$mon,$year\n";
	$time = timelocal(0,0,0,$mday,$mon,$year);
	print "timestamp created is $time\n";
	return ($time);
}
...
print "(Start Varaibles: $startMonth,$startDay,$startYear)\n";
$start = createTimeStamp($startMonth,$startDay,$startYear);
print "(End Variables: $endMonth,$endDay,$endYear)\n";
$end = createTimeStamp($endMonth,$endDay,$endYear);

($smon, $smday, $syear) = extractDate($start);
($emon, $emday, $eyear) = extractDate($end);

print "starting at month = $smon, day = $smday, year = $syear: ending at $emon, $emday, $eyear\n\n";
